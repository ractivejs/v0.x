...
Sometimes you have a nested context that puts a parent completely out of reach by keypath e.g.:

```html
\{{#each foos}}
  \{{#with ~/bar}}
    \{{.someBarThing}}
    How do I get to the current foo?
  \{{/with}}
\{{/each}}
```
...
5. `@this` - the current Ractive instance e.g. the current component or the Ractive root. You can access any properties or methods available on a Ractive instance with this, and due to the way capture works, you can also use `@this.get('...')` in a binding and it will update with the target keypath.
  * `@` followed by a `.` is shorthand for `@this` e.g. `@this.set` shorthand would be `@.set`.
6. `@shared` - a model that is globally shared across all Ractive instances. It's effectively a version of `@global` that can't be trampled by other scripts that happen to be executing on the page.

### Instance parents and data

Instance references (`@this`) with a path that resolves to a Ractive instance e.g. `@this.parent` or `@.root` will resolve to the actual Ractive model as though `@this` were being referenced from the template of the parent instance. This avoids creating disjoint model trees across instances that can get out of sync.

Further, there is a special `data` property available to instance models that resolves to the actual root model on the instance. So `@this.parent.data.foo` resolves to the `~/foo` model on the current instance's parent. This also avoids creating complex expressions in your viewmodel that can get out of sync across instances. If any parents happen to change, for instance by detaching and reattaching to a new parent, the references are kept up-to-date as well.
...
* `../` means the parent keypath of the current context and may also be stacked and have a following path.
