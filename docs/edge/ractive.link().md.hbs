...
> > ### **options** *`Object`*
> > Options to use when creating the link:
> > * **ractive** *`Ractive instance`* - the target Ractive instance against which to create the link. Cross-instance links are the same mechanism used by component mappings to share data between parents and their child components.
