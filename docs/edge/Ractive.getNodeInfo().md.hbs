...
> ### info.decorators
> This property gives you access to any decorator handles that are currently installed on the element. The decorators are available by name, so `<div as-foo>` would expose the `foo` decorator as `info.decorators.foo`.
...
> ### info.observe( keypath, callback[, options] )
> Creates an observer for the given keypath, or keypaths if keypath is an object. See {{{createLink 'ractive.observe()'}}}.

> ### info.observeOnce( keypath, callback[, options] )
> Creates an observer for the given keypath, or keypaths if the keypath is an object. See {{{createLink 'ractive.observeOnce()'}}}.
...
> ### info.raise( name, event, ...arguments )
> Trigger an event directive callback for `name` using the given event and arguments. This does not bubble and _only_ triggers the event directive listener handled by Ractive, and no DOM event is generated or simulated. This is particularly handy for decorators.
