...
Partials may also be defined inline as part of their expression, resulting in an entirely dynamic partial. If the result of a partial expression is an object with a `t` array e.g. `{ t: [] }` or a `template` string e.g. `{ template: '' }`. The former expects a pre-parsed template, like the result of a call to {{{createLink 'Ractive.parse'}}}. The latter handles the parsing for you, but cannot be used with Ractive versions that do not include a parser (runtime-only). Here's an example where the template that is displayed can be edited in a textarea:

```html
<textarea value="\{{.template}}" lazy="500"></textarea>
<br/>
\{{> { template: .template } }}
```

If `template` was set to `<input value="\{{.input}}" />`, then the partial would output an input element that was properly bound to the `.input` keypath.
