...
We're going to create a `flash` transition, that flashes text green on intro, and red on outro. To skip ahead and see the result, [open the JSFiddle](http://jsfiddle.net/madgvox/peLaq5pd).
...
You can see the finished product at [this JSFiddle](http://jsfiddle.net/madgvox/peLaq5pd).
