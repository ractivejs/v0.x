...
You can subscribe a handler to more than one event by seperating event names with a space:
...
* `this.event.context` - the value of `this.get(event.keypath)` or `ractive.data` for non-proxy events
...
Note that returning `false` has a duel purpose of both cancelling the view hierarchy {{{createLink 'Event bubbling' 'event bubbling'}}} __as well as__ cancelling the DOM Event if the event was DOM-based.
